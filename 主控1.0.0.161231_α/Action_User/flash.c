...
/**
  ...
  * @retval None
  */
void FlashWriteGunPosData(float *data1, float *data2, float *data3, uint32_t len1, uint32_t len2, uint32_t len3)
{
	uint32_t count;
	FLASH_Unlock();
	FLASH_ClearFlag(FLASH_FLAG_EOP | FLASH_FLAG_OPERR | FLASH_FLAG_WRPERR | FLASH_FLAG_PGAERR | FLASH_FLAG_PGPERR | FLASH_FLAG_PGSERR);
...
void Flash_Init(void)
{
	uint32_t len = FlashFloatDataLen;
	/* 读取FLASH中保存的数据，并将其存到RAM里 */
	Flash_ReadFloat((float *)gGunPosDatabaseFromFlash, len);
